...
object KfUtils09 {
  ...
  def getEarliestOffsetsI(kafkaConsumer: KafkaConsumer[String, String], topicPartitions: util.Collection[TopicPartition]): Map[TopicPartition, Long] = {
    topicPartitions.asScala.foreach(a => kafkaConsumer.seekToBeginning(Seq(a).asJava))
    topicPartitions.asScala.map { p => (p, kafkaConsumer.position(p)) }.toMap
  }

  def getLatestOffsetsI(kafkaConsumer: KafkaConsumer[String, String],topicPartitions: util.Collection[TopicPartition]): Map[TopicPartition, Long] = {
    topicPartitions.asScala.foreach(a => kafkaConsumer.seekToEnd(Seq(a).asJava))
    topicPartitions.asScala.map { p => (p, kafkaConsumer.position(p)) }.toMap
  }

  def getCommittedOffsetsI(kafkaConsumer: KafkaConsumer[String, String], topicPartitions: util.Collection[TopicPartition]): Map[TopicPartition, Long] = {
  ...
  def getCommittedOffsets(topicNames: util.Collection[String], kafkaConfig: Properties): Option[Map[TopicPartition, Long]] = {
    getOffset(topicNames, kafkaConfig, getCommittedOffsetsI)
  }
...
//getCommittedOffsets(topicNames, prop)
